// The API key lives server-side in the Vercel function (api/send-email.js).
// The frontend only calls our own /api/send-email endpoint — no CORS issues.
...
function buildEmailHTML({ date, sleepHours, focusMins, moodScore, habitUpdates, habits, userName }) {
  ...
  const completedCount = (habitUpdates || []).filter(update => {
    ...
    return habit.isProgressType ? Number(update.value) >= habit.targetValue : Boolean(update.value);
  }).length;
...
<title>Daily Summary — SleekHabits</title></head>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  <tr>
    <td style="background:linear-gradient(135deg,#0066FF 0%,#7c3aed 100%);border-radius:16px 16px 0 0;padding:36px 40px;text-align:center;">
      ...
      <h1 style="margin:0;color:#fff;font-size:22px;font-weight:800;letter-spacing:-0.5px;">Daily Habit Summary</h1>
  ...
  <tr>
    <td style="background:#080d1a;border-radius:0 0 16px 16px;padding:20px 40px;text-align:center;">
      <p style="margin:0;color:#334155;font-size:11px;">Sent by <strong style="color:#0066FF;">SleekHabits</strong> · Your personal habit tracker</p>
    </td>
  </tr>
</table>
</td></tr></table>
...
/**
 * Sends the daily summary email by calling our own Vercel serverless function
 * at /api/send-email — avoids CORS entirely since it's same-origin.
 */
export async function sendDailySummaryEmail({
  ...
  console.log('Recipient (auth email passed to server):', toEmail);
  console.log('Date:', date, '| Sleep:', sleepHours, '| Focus:', focusMins, '| Mood:', moodScore);
  console.log('Habits:', habitUpdates?.length ?? 0);
  ...
  try {
    // POST to our own Vercel serverless function — no CORS, API key stays server-side
